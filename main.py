...
class KBManagerPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        # pending_delete: key = f"{umo}:{sender_id}" -> dict
    ...
    def _register_official_page_api_if_available(self) -> None:
        ...
        if not hasattr(self.context, "register_web_api"):
            logger.warning(
                "[kb_manager] 当前 AstrBot 不支持 register_web_api，WebUI 页面 API 未注册"
            )
            return
        try:
    ...
    async def _resolve_kb(self, token: str):
        ...
        matches = [
            h
            for h in helpers
            if token in (h.kb.kb_name or "") or token in (h.kb.kb_id or "")
        ]
        if len(matches) == 1:
    ...
    async def _collect_delete_targets(self, kb_helper, tokens: list[str]):
        """批量解析多个删除目标：支持编号 / doc_id(前缀) / 文件名(精确或包含)"""
        docs = await kb_helper.list_documents(offset=0, limit=500)
        if not docs:
            return None, "这个知识库里还没有文档。"
    ...
    async def cmd_list_kb(self, event: AstrMessageEvent):
        ...
        for i, h in enumerate(helpers, 1):
            ...
            lines.append(
                f"{i}. {emoji} {name}\n"
                f"   文档 {kb.doc_count} · 块 {kb.chunk_count} · ID {_short_id(kb.kb_id)}"
                f"{status}"
            )
            if desc:
    ...
    async def cmd_list_docs(self, event: AstrMessageEvent):
        ...
        lines = [
            f"📁 {kb.emoji or '📘'} {kb.kb_name} 的文件",
            f"共 {total} 个 · 第 {page}/{total_pages} 页",
        ]
        if not docs:
            ...
            for i, d in enumerate(docs, start=offset + 1):
                lines.append(
                    f"{i}. {d.doc_name}\n"
                    f"   类型 {d.file_type} · {_fmt_size(d.file_size)} · 块 {d.chunk_count} · ID {_short_id(d.doc_id)}"
                )
        lines.append(
    ...
    async def cmd_doc_detail(self, event: AstrMessageEvent):
        ...
        lines = [
            f"📄 文档详情 · {kb_helper.kb.kb_name}",
            f"名称：{doc.doc_name}",
            f"ID：{doc.doc_id}",
            f"类型：{doc.file_type}",
            f"大小：{_fmt_size(doc.file_size)}",
            f"分块数：{doc.chunk_count}",
            f"媒体数：{getattr(doc, 'media_count', 0)}",
            f"路径：{doc.file_path}",
        ]
        if created:
    ...
    async def cmd_delete_doc(self, event: AstrMessageEvent):
        ...
        if len(parts) < 2:
            yield event.plain_result(
                "用法：知识库删除文件 <知识库名称|编号> <文档编号|ID|文件名>\n"
                "删除前会要求你发送「知识库确认删除」。"
            )
            return
        ...
        if getattr(kb_helper, "init_error", None):
            yield event.plain_result(
                f"该知识库初始化异常，暂不能删除：{kb_helper.init_error}"
            )
            return
        ...
        self._pending_delete[key] = {
            ...
            "docs": [{"doc_id": doc.doc_id, "doc_name": doc.doc_name}],
            "expire_at": expire,
            "sender_id": str(event.get_sender_id()),
        }
        yield event.plain_result(
            "⚠️ 即将删除知识库文档（不可恢复）\n"
            f"知识库：{kb_helper.kb.kb_name}\n"
            f"文件：{doc.doc_name}\n"
            f"ID：{doc.doc_id}\n"
            f"请在 {self._confirm_seconds()} 秒内发送：知识库确认删除\n"
            "取消请发：知识库取消删除"
        )
    ...
    async def cmd_batch_delete(self, event: AstrMessageEvent):
        ...
        if len(parts) < 2:
            yield event.plain_result(
                "用法：知识库批量删除 <知识库名称|编号> <文档编号/ID/文件名...>\n"
    ...
    async def cmd_confirm_delete(self, event: AstrMessageEvent):
        ...
        docs = pending.get("docs") or [
            {"doc_id": pending.get("doc_id"), "doc_name": pending.get("doc_name", pending.get("doc_id"))}
        ]
        ...
        try:
            ...
            logger.info(
                f"[kb_manager] 用户 {event.get_sender_id()} 批量删除 kb={kb_id} ok={len(ok_list)} fail={len(fail_list)}"
            )
            yield event.plain_result("\n".join(lines))
        except Exception as e:
    ...
    async def cmd_cancel_delete(self, event: AstrMessageEvent):
        ...
        if key in self._pending_delete:
            ...
            docs = p.get("docs") or [
                {"doc_id": p.get("doc_id"), "doc_name": p.get("doc_name", p.get("doc_id"))}
            ]
