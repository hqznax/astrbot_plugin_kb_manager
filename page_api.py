...
def _download_response(data: bytes, media_type: str, filename: str) -> StarletteResponse:
    ...
    return StarletteResponse(content=data, media_type=media_type, headers={"Content-Disposition": disposition})
...
def _doc_to_dict(doc) -> dict[str, Any]:
    if hasattr(doc, "model_dump"):
        ...
        data = {"doc_id": getattr(doc, "doc_id", None), "kb_id": getattr(doc, "kb_id", None), "doc_name": getattr(doc, "doc_name", None), "file_type": getattr(doc, "file_type", None), "file_size": getattr(doc, "file_size", None), "file_path": getattr(doc, "file_path", None), "chunk_count": getattr(doc, "chunk_count", 0), "media_count": getattr(doc, "media_count", 0), "created_at": getattr(doc, "created_at", None), "updated_at": getattr(doc, "updated_at", None)}
...
def _kb_to_dict(helper) -> dict[str, Any]:
    ...
    if hasattr(kb, "model_dump"):
        ...
        data = {"kb_id": getattr(kb, "kb_id", None), "kb_name": getattr(kb, "kb_name", None), "description": getattr(kb, "description", None), "emoji": getattr(kb, "emoji", "📘"), "doc_count": getattr(kb, "doc_count", 0), "chunk_count": getattr(kb, "chunk_count", 0)}
    for key in ("created_at", "updated_at"):
        val = data.get(key)
        if hasattr(val, "isoformat"):
            data[key] = val.isoformat()
...
class PluginPageApi:
    ...
    def register_routes(self) -> None:
        ...
        routes = [
            ...
            (f"{PAGE_API_PREFIX}/documents", self.list_documents, ["GET"], "KB Manager list documents"),
            (f"{PAGE_API_PREFIX}/document", self.get_document, ["GET"], "KB Manager get document"),
            (f"{PAGE_API_PREFIX}/document/content", self.get_document_content, ["GET"], "KB Manager get document content"),
            (f"{PAGE_API_PREFIX}/document/update", self.update_document, ["POST"], "KB Manager update document"),
            (f"{PAGE_API_PREFIX}/document/delete", self.delete_document, ["POST"], "KB Manager delete document"),
            (f"{PAGE_API_PREFIX}/document/batch_delete", self.batch_delete_document, ["POST"], "KB Manager batch delete documents"),
            (f"{PAGE_API_PREFIX}/document/create", self.create_document, ["POST"], "KB Manager create document"),
            (f"{PAGE_API_PREFIX}/document/download", self.download_document, ["GET"], "KB Manager download document as file"),
            (f"{PAGE_API_PREFIX}/document/batch_download", self.batch_download_documents, ["GET", "POST"], "KB Manager batch download documents as zip"),
        ]
    ...
    async def list_documents(self):
        try:
            ...
            docs = await helper.list_documents(offset=offset, limit=page_size, search=search)
            total = await helper.count_documents(search=search)
            return _ok({"items": [_doc_to_dict(d) for d in docs], "page": page, "page_size": page_size, "total": total, "kb": _kb_to_dict(helper)})
        except Exception as e:
    ...
    async def _load_content(self, helper, doc_id: str) -> tuple[str, list[dict]]:
        ...
        offset = 0
        while offset < total:
            batch = await helper.get_chunks_by_doc_id(doc_id=doc_id, offset=offset, limit=page_size)
            if not batch:
                break
            chunks.extend(batch)
            offset += len(batch)
    ...
    async def get_document_content(self):
        try:
            ...
            return _ok({"document": _doc_to_dict(doc), "content": content, "chunk_count": len(chunks), "kb": _kb_to_dict(helper), "note": "正文由向量文本块拼接还原；若原文件含复杂格式，编辑后会按纯文本重新分块入库。"})
        except Exception as e:
            logger.error(f"[kb_manager] get_document_content failed: {e}", exc_info=True)
            return _error(str(e))
    ...
    async def update_document(self):
        try:
            ...
            new_doc = await helper.upload_document(file_name=file_name, file_content=content_text.encode("utf-8"), file_type=file_type, chunk_size=int(chunk_size), chunk_overlap=int(chunk_overlap))
            try:
                await helper.delete_document(doc_id)
            except Exception as del_err:
                logger.error(f"[kb_manager] update: new uploaded but old delete failed: {del_err}", exc_info=True)
                return _error(f"新文档已保存，但旧文档删除失败（可能暂时并存）：{del_err}")
            try:
                await helper.refresh_kb()
            except Exception:
                pass
            return _ok({"message": "文档已更新（已重建索引）", "old_doc_id": doc_id, "document": _doc_to_dict(new_doc)})
        except Exception as e:
    ...
    async def create_document(self):
        try:
            ...
            chunk_size = getattr(helper.kb, "chunk_size", None) or 512
            chunk_overlap = getattr(helper.kb, "chunk_overlap", None) or 50
            new_doc = await helper.upload_document(file_name=file_name, file_content=str(content).encode("utf-8"), file_type=file_type, chunk_size=int(chunk_size), chunk_overlap=int(chunk_overlap))
    ...
    async def download_document(self):
        try:
            ...
            return _download_response(content.encode("utf-8"), "application/octet-stream", safe_name)
        except Exception as e:
    ...
    async def batch_download_documents(self):
        try:
            ...
            if kb_id and raw_ids:
                ...
                doc_ids = [str(x).strip() for x in ids if str(x).strip()] if isinstance(ids, list) else [x.strip() for x in str(ids).split(",") if x.strip()]
            if not kb_id:
                return _error("缺少 kb_id")
            if not doc_ids:
    ...
    async def batch_delete_document(self):
        try:
            ...
            if not ok_list and fail_list:
                return _error(f"全部删除失败：{fail_list[0].get('error', '未知错误')}")
            message = f"删除成功 {len(ok_list)} 个"
            if fail_list:
                message += f"，失败 {len(fail_list)} 个"
            return _ok({"message": message, "ok": ok_list, "fail": fail_list, "success_count": len(ok_list), "fail_count": len(fail_list)})
        except Exception as e:
